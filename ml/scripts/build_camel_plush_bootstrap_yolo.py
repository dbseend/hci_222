...
DEFAULT_SOURCE = ROOT / "dataset_sources" / "camel_doll_web"
DEFAULT_OUT = ROOT / "dataset_sources" / "camel_doll_yolo_bootstrap"
IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png", ".webp"}
SPLITS = ("train", "valid", "test")
SPLIT_RATIOS = {"train": 0.7, "valid": 0.2, "test": 0.1}
SEED = 222
DEFAULT_CLASS_NAME = "camel_doll"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"camel doll review dataset directory. Default: {DEFAULT_SOURCE}",
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=f"Class/image folder name under source/images. Default: {DEFAULT_CLASS_NAME}",
    )
    parser.add_argument(
        "--image-size",
        type=int,
...
def source_images(source: Path, class_name: str = DEFAULT_CLASS_NAME) -> list[Path]:
...
def build_dataset(args: argparse.Namespace) -> dict:
    images = source_images(args.source, args.class_name)
    ...
    write_dataset_files(args.out, args.source, args.class_name, summary)
    return summary
...
def write_dataset_files(out: Path, source: Path, class_name: str, summary: dict) -> None:
...
This is a weak-labeled training bootstrap dataset for `{class_name}`.
